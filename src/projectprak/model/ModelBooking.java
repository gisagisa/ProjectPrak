...
public class ModelBooking {
    ...
    public Booking getOne(String ids) {
        try {
            String query = "Select * from `booking` where `kode` ='" + ids+"'"; //proses pengambilan data
            connector.statement = connector.koneksi.createStatement();
            ResultSet resultSet = connector.statement.executeQuery(query); //result isinya tabel belum berupa string
            while (resultSet.next()) { //konversi tabel ke string
                int id = resultSet.getInt("id");
    ...
    public String update_status(String kode, int status) {
        String message = "Bad Request";
        try {
            String query = "UPDATE booking SET status = " + status + " WHERE kode = '" + kode + "'";
            connector.statement = connector.koneksi.createStatement();
            connector.statement.executeUpdate(query);
            message = "Queri Berhasil";
            connector.statement.close();
    ...
    public String delete(String kode) {
        String message = "Bad Request";
        try {
            String query = "DELETE FROM booking WHERE kode = '" + kode + "'";
            connector.statement = connector.koneksi.createStatement();
            connector.statement.executeUpdate(query);
            message = "Queri Berhasil";
            connector.statement.close();
